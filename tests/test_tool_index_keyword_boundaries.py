...
"unreadable". That bloated the tool set with irrelevant document/serve
...
def test_genuine_keywords_still_force_include():
    ...
    # Serving lifecycle is deferred — the "serve" intent now surfaces only the
    # read-only cookbook visibility tools.
    assert "list_cookbook_servers" in ti.get_tools_for_query("serve the model")
