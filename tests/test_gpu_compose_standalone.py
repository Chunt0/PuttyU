...
base compose with only the matching overlay merged into the ``puttyu``
...
def _merge_overlay_into_base(base: dict, overlay: dict) -> dict:
    """Build the expected standalone config: base + overlay on puttyu only."""
...
# --- Non-puttyu services and volumes untouched ---------------------------


@pytest.mark.parametrize("standalone_path", [NVIDIA_STANDALONE, AMD_STANDALONE])
def test_non_puttyu_services_match_base(base, standalone_path):
...
# --- puttyu = base service + only the overlay additions ------------------


def test_nvidia_puttyu_adds_only_overlay(base):
    standalone = _load(NVIDIA_STANDALONE)
...
def test_amd_puttyu_adds_only_overlay(base):
    standalone = _load(AMD_STANDALONE)
    svc = standalone["services"][SERVICE]
    base_svc = base["services"][SERVICE]
