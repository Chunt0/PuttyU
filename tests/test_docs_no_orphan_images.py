...
referenced by no tracked text file. A stray screenshot dropped in by a future
PR would fail; an empty docs/ (the legacy puttyU hero image and preview clips
were deleted with the puttyU rebrand — the README now uses snapshots/) trivially
passes. snapshots/ itself is exempt: it is the capture-spec's full output for
human review, intentionally committed whole even though the README links only
the best shots.
"""
import subprocess
...
def test_no_orphan_images_in_docs():
    ...
    if not docs_images:
        return  # nothing under docs/ to orphan-check
