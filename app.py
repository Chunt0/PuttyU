...
app = FastAPI(
    title="puttyU",
    description="puttyU — AI tutoring workspace with chat, memory, research, and multi-modal capabilities",
    version="1.0.0",
)
...
if AUTH_ENABLED:
    ...
    def _is_trusted_loopback(request: Request) -> bool:
        ...
        slip past LOCALHOST_BYPASS or spoof the internal-tool path. puttyU's own
        ...
        async def dispatch(self, request: Request, call_next):
            ...
            try:
                ...
                if _hdr and secrets.compare_digest(_hdr, _ITT) and _is_trusted_loopback(request):
                    ...
                    # X-PuttyU-Owner, attribute the request to that user only
                    # if they exist. Authorization checks remain separate; this
                    # is just owner attribution for notes/calendar/etc.
                    _impersonate = (request.headers.get("X-PuttyU-Owner") or "").strip()
...
if os.path.isdir(WEB_DIST):
    app.mount("/assets", StaticFiles(directory=abs_join(WEB_DIST, "assets")), name="web-assets")
    _web_fonts = abs_join(WEB_DIST, "fonts")
    if os.path.isdir(_web_fonts):
        app.mount("/fonts", StaticFiles(directory=_web_fonts), name="web-fonts")


def _serve_web_app() -> HTMLResponse:
    """Serve the puttyU SPA shell. Client-side routing (react-router) owns
    /login, /models, /notes, … so every non-API, non-asset GET returns this.
    Falls back to a clear message if the bundle was never built."""
    if not os.path.exists(WEB_INDEX):
        return HTMLResponse(
            "<h1>puttyU UI not built</h1><p>Run <code>cd web && bun run build</code> "
            "(or rebuild the Docker image) to generate web/dist.</p>",
            status_code=503,
        )
...
@app.get("/putty-blob.svg")
async def serve_putty_blob():
    return FileResponse(abs_join(WEB_DIST, "putty-blob.svg"))

@app.get("/login")
async def serve_login(request: Request):
    # react-router renders the Login screen client-side; just serve the shell.
    return _serve_web_app()
...
async def _startup_event():
    ...
    if _tasks_inprocess not in ("0", "false", "no", "off", ""):
        ...
        logger.info(
            "In-process task scheduler disabled (PUTTYU_INPROCESS_TASKS=0); "
