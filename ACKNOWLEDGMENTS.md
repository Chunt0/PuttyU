...
puttyU stands on the shoulders of a lot of open-source work. This file
credits the projects whose code, assets, or designs are included in or
adapted by this repository, and notes their licenses.

puttyU is a fork of **[Odysseus](https://github.com/pewdiepie-archdaemon/odysseus)**
(a self-hosted AI workspace) — the entire Python backend and the original
frontend derive from it. Copyright © 2025 the Odysseus Contributors, MIT
License (preserved in [`LICENSE`](LICENSE)).
...
and run alongside puttyU on `docker compose up`. They are not modified —
just composed.
...
puttyU talks to these over the network/API. They are **not** distributed
...
- **`caldav`** (Python lib) is **dual-licensed GPL-3.0-or-later OR Apache-2.0**.
  puttyU uses it under **Apache-2.0**, which is permissive and MIT-compatible.
...
Most of this project's code was written *with* AI models, not just by a human.
