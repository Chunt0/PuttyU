...
INTERNAL_TOOL_TOKEN = app_env("INTERNAL_TOKEN") or secrets.token_hex(32)
INTERNAL_TOOL_HEADER = "X-PuttyU-Internal-Token"
...
def require_admin(request: Request):
    ...
    # (a) header-direct (caller set X-PuttyU-Internal-Token), or
...
class SecurityHeadersMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if is_report:
            ...
            # The React SPA sets runtime `style=""` attributes (style={{…}}),
            # which CSP treats as inline styles. Since inline styles don't
            # execute script, the residual risk is visual-only.
            response.headers["Content-Security-Policy"] = (
                "default-src 'self'; "
