...
| **M0.3 — the shell, for real** | Odysseus-fidelity shell from the kit: sidebar, **dockable window manager**, theme picker (16+1), **Cmd-K palette**, slash-command composer. **Sub-chunk at entry:** first extract a concrete fidelity checklist from `ODYSSEUS-REF/` (SPEC §13 O12), then build against it — this is the largest M0 chunk. | **UX-fidelity checklist met (O12)** — verified side-by-side vs `ODYSSEUS-REF/` (SPEC §6 / M0 DoD). |
