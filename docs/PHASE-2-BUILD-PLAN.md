...
|---|---|---|
| Backend tests | `.venv/bin/python -m pytest -q -m "not quarantine"` | **2335 passed, 1 skipped** (T4 + T5 todos/dashboard in) |
| Fitness gates | `bash .fitness/run-all.sh` | **6a–6f all pass** |
| TS types | `cd web && bunx tsc --noEmit` | clean |
| Lint | `cd web && bun run lint` | clean |
| Vitest | `cd web && bun run test` | **192 passed** (T4b + T5 dashboard in) |
| Playwright | `cd web && bun run e2e` | **32 passed, 2 skipped** (T4b + T5 dashboard in) |
