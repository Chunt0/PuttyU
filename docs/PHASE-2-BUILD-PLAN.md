...
| Playwright | `cd web && bun run e2e` | **39 passed, 2 skipped** (T6 complete) |
...
| **T5** | Dashboard + todos + schedule miner + persona/dial + integrity + Cmd-K + cost meter + session-summary notes | ✅ **DONE (7/7 verticals)** — todos (`todo` table + typed CRUD), the read-only `/api/dashboard` aggregator (review-count/weak-spots/insights/reading; `recent_insights` graph door; never-500, no-mint, Gate-6f clean), and the Dashboard landing surface (`web/src/features/dashboard/`: cards deep-linking calendar/todos/Review/Gym/Progress/PDF/resume, `useGymStore` weak-spot→Gym preload, login-lands-on-dashboard). Adversarially reviewed (5 bugs fixed w/ regressions). Pinned decisions: `docs/T5-DASHBOARD-CONTRACT.md`. **Vertical 2 (schedule miner, F2) done** — `src/schedule/` engine + `routes/schedule_routes.py` (`POST /api/schedule/{id}/mine` read-only extraction + diff; `POST .../apply` the only writer), `CalendarEvent.provenance` column, idempotent re-mine via content-hash `proposal_key` + occurrence ordinal, ambiguity asked-not-guessed; frontend `web/src/features/schedule/` confirm-first review sheet (prune/edit/resolve, provenance chips) from a "Mine schedule" button on a material. Adversarially reviewed (5 bugs fixed incl. single-user event-dup + bad-date-500). Pinned: `docs/T5-MINER-CONTRACT.md`. **Vertical 3 (persona + dial + integrity, F10) done** — `src/tutor_persona.py` builds a course behavior frame (patient/Socratic, cites library, admits uncertainty, never shames) shaped by the per-course dial (scaffolding/pace/tone, zero-config defaults) + a course-type lean + the baked-in integrity stance, injected via `course_system_messages` (explain mode suppresses it); the `web/src/features/courses/CourseSettings.tsx` dial panel in CourseLanding writes the dial (merged) via the existing course PATCH. **Vertical 4 (session-summary notes, F9) done** — on-demand `POST /api/sessions/{id}/summary` (`src/session_summary.py` + `routes/session_summary_routes.py`): reads the transcript + the graph ground-truth (`student_context`), router-drafts (tier=light, free, degrades to no_llm on any provider failure) an editable `source="agent"` course note (Covered / clicked / shaky / citations; never invents citations); too_short/no_llm write nothing. Closed the `Note.course_id` gap (surfaced in create/serialize/list). A "Summarize" button in the chat header → toast + opens Notes. Adversarially reviewed (1 med fixed: LLM-failure → no_llm; + test gaps). **Vertical 5 (Cmd-K global search, F11) done** — new `GET /api/cmdk` (`routes/global_search_routes.py`): owner-scoped cross-entity search across courses/notes/materials/sessions/todos/concepts (concepts via a new `queries.search_concepts` one-door, Gate 6f), flat `{kind,...}` results with per-bucket degrade (never-500); each concept result resolves its owning course from its sources so the trajectory door lands right. Frontend: an always-mounted ⌘/Ctrl-K palette (`web/src/features/search/`) with grouped results + keyboard nav, deep-linking each kind to its surface (concept→trajectory via a new `progressStore`, material→PDF, session→resume, todo→course, course→tab, note→Notes). Adversarially reviewed (2 reviewers: 1 real concept-course bug + a preselect race fixed). **Vertical 7 (typed math, F4) done** — a self-hosted KaTeX renderer wired into the
one shared `<Markdown>` (remark-math + rehype-katex, no CDN; single-`$` inline math
OFF so currency prose never mangles, KaTeX bounded against layout bombs) + a reusable
`web/src/components/MathInput.tsx` "insert equation" affordance (LaTeX field + live
preview, emits block `$$…$$`) mounted across all answer surfaces (chat composer,
Review, Gym, Exam, Calibration, Explain). Backend untouched. Adversarially reviewed
(security clean; fixed a currency-mangling regression + newline/layout-bomb/a11y).
...
| **T6** | Worksheet grading contract + graph hook + **canvas workspace** | ✅ **DONE** — T6b canvas: `web/src/features/canvas/` (Pointer-Events draw surface w/ pressure, templates blank/ruled/grid/axes, pen/eraser/undo/clear; eraser composites to an opaque sheet; "send to tutor" = the CameraCapture toBlob→onAccept door → existing upload, mounted inline in chat/gym/worksheet; standalone window saves a PNG material; stroke-JSON sidecar reopen-editable is a noted follow-up). Adversarially reviewed (1 high fixed: transparent-erase hole). Pinned: `docs/T6-CANVAS-CONTRACT.md`. — T6a done — `grade_worksheet` (multi-problem, line-referenced: what's right / FIRST error / nudge in guide mode / cite the concept; closed-world concept resolution; writes `source="worksheet"` evidence w/ error-pattern via the graph one-door → the weakness bump IS the declarative follow-up review item; never grades blind — no image → no LLM/evidence) + `POST /api/practice/worksheet` + a calm "Worksheet" view (photo/upload → per-problem feedback + citation doors). Adversarially reviewed (1 high fixed: blind-grade guard). Pinned: `docs/T6-WORKSHEET-CONTRACT.md`. **Remaining: the canvas workspace** (T6b). |
