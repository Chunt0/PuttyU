...
|---|---|---|
| Backend tests | `.venv/bin/python -m pytest -q -m "not quarantine"` | **2367 passed, 1 skipped** (T4 + T5 dashboard/miner/persona/summary in) |
| Fitness gates | `bash .fitness/run-all.sh` | **6a–6f all pass** |
| TS types | `cd web && bunx tsc --noEmit` | clean |
| Lint | `cd web && bun run lint` | clean |
| Vitest | `cd web && bun run test` | **211 passed** (+ T5 dashboard/miner/dial/summary) |
| Playwright | `cd web && bun run e2e` | **35 passed, 2 skipped** (+ T5 dashboard/miner/summary) |
