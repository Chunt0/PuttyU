#!/usr/bin/env python3
"""puttyU — first-time setup script.
...
from core.app_env import app_env
...
def create_default_admin():
    ...
    try:
        ...
        username = app_env("ADMIN_USER", "").strip().lower()
        password = app_env("ADMIN_PASSWORD", "").strip()

        if username and password:
            # Both provided via env — use them directly
            pass
        elif sys.stdin.isatty() and not app_env("SKIP_ADMIN_PROMPT"):
        ...
        if sys.stdin.isatty() and not app_env("ADMIN_PASSWORD"):
            print(f"  [ok] Admin account created ({username})")
        else:
            print(f"  [ok] Initial admin user created ({username})")
            if not app_env("ADMIN_PASSWORD"):
                print(f"        Temporary password: {password}")
                print(f"        ** Change it after first login. Set PUTTYU_ADMIN_PASSWORD to choose your own. **")
        return "created"
    except ImportError:
...
def check_deps():
    ...
    if shutil.which("tmux") is None:
        ...
        print("         Install it with: sudo apt install tmux")
    else:
...
def main():
    ...
    print("\n=== Setup complete ===")
    # A wrapper launcher can set PUTTYU_SKIP_RUN_HINT (legacy: PUTTYU_SKIP_RUN_HINT)
    # when it starts the server itself (on its own port), to avoid printing a
    # contradictory URL here.
    if not app_env("SKIP_RUN_HINT"):
