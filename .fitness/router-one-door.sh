...
# call site; calls declare a task profile and go through src/model_router.py.
#
...
ALLOWED_RE='backend/(src/model_router\.py|tests/|scripts/)'

hits=$(
  grep -rnE "$PATTERN" \
    "$ROOT/backend/app.py" "$ROOT/backend/core" "$ROOT/backend/routes" \
    $([ -d "$ROOT/backend/src" ] && echo "$ROOT/backend/src") \
...
if [ -n "$hits" ]; then
  echo "$hits" >&2
  fail "router-one-door: hardcoded model name at a call site — declare a task profile via src/model_router.py"
fi
