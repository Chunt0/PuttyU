...
def setup_research_routes(research_handler, session_manager=None) -> APIRouter:
    ...
    @router.get("/api/research/status/{session_id}", response_model=ResearchStatusResponse)
    ...
    @router.get("/api/research/library", response_model=ResearchLibraryResponse)
    ...
    async def research_start(body: ResearchStartRequest, request: Request):
        ...
        if user == "internal-tool":
            tool_owner = (request.headers.get("X-PuttyU-Owner") or "").strip()
