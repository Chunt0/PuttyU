...
def _maybe_cascade_calendar_event(task) -> None:
    ...
    if task.owner:
        headers["X-PuttyU-Owner"] = task.owner
...
def setup_task_routes(task_scheduler) -> APIRouter:
    ...
    async def list_output_targets(request: Request):
        ...
        # action — match by verb in the tool name. Also exclude read/search/list
        # tools whose names happen to start with a delivery verb.
