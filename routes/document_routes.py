...
def setup_document_routes(session_manager, upload_handler=None) -> APIRouter:
    ...
    async def render_pdf(doc_id: str, request: Request):
        ...
        try:
            ...
            if annotations:
                ...
                try:
                    stamp_annotations(out_path, annotated_path, annotations)
    ...
    async def export_pdf(doc_id: str, request: Request):
        ...
        try:
            ...
            # Signature-type fields are excluded — they can't be text-filled
            # and stored-signature stamping was removed with the feature.
            text_values: dict = {
                name: raw for name, raw in all_values.items()
                if name not in sig_field_names
            }
            ...
            # Burn freeform annotations (Text/Check drops) on top.
            annotations = parse_markdown_annotations(doc.current_content or "")
            if annotations:
                annotated_path = tempfile.NamedTemporaryFile(suffix=".pdf", delete=False).name
                _to_unlink.append(annotated_path)
