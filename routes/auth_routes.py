...
from src.request_models import LoginResponse, LogoutResponse, AuthStatusResponse
import asyncio
import logging
import os
...
def setup_auth_routes(auth_manager: AuthManager) -> APIRouter:
    ...
    async def test_integration_route(integration_id: str, request: Request):
        ...
        if preset == "ntfy":
            ...
            # base URL (e.g. `http://host:8091/puttyu`) — otherwise
            # the topic gets appended after the path and we publish to
            # `/puttyu/puttyu` (which ntfy 404s on). ntfy itself
            ...
            headers = {
                "Title": "puttyU connectivity test",
            ...
            try:
                async with httpx.AsyncClient(timeout=8.0) as client:
                    r = await client.post(
                        full_url,
                        content="Connectivity test from puttyU. If you see this on your phone, ntfy is wired up correctly.",
                        headers=headers,
                    )
                if r.is_success:
