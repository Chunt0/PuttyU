...
def _container_loopback_reachable(base_url: str, timeout: float = 0.2) -> bool:
    ...
    This distinguishes "a model server running alongside puttyU in the same
...
def _rewrite_loopback_for_docker(base_url: str, *, container_local: bool = False) -> str:
    ...
    default) otherwise targets the puttyU container itself, so the probe gets
    a connection error and the endpoint is rejected with a misleading "No
    models found for that provider/key".

    Cookbook local serves are the opposite case: puttyU started the model
        ...
        if 300 <= r.status_code < 400:
            ...
            if loc.startswith("/login") or "/login" in loc:
                return {
                    ...
                    "error": "That is puttyU, not a model server. Use the Ollama URL, usually http://host.docker.internal:11434/v1 in Docker.",
                }
...
def setup_model_routes(model_discovery):
    ...
    def create_model_endpoint(
        ...
        # server. Cookbook local serves are launched inside puttyU itself, so
    ...
    def get_default_chat(request: Request):
        ...
        _user_prefs = (_load_for_user(_user) or {}) if _user else {}
        ...
        # Admin/single-user falls back to global settings ONLY when no per-user pick (so the
        # Providers default takes effect); non-admins never read globals (the admin-pick leak).
        if (_is_admin or not _user) and not ep_id:
            ep_id, model = settings.get("default_endpoint_id", ""), settings.get("default_model", "")
