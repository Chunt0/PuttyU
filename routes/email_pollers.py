                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in puttyU (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in puttyU" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "puttyu_kind" if "puttyu_kind" in cols else "'scheduled' AS puttyu_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                puttyu_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-PuttyU-Origin"] = "puttyu-ui"
                outer["X-PuttyU-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", puttyu_kind or "scheduled")[:64]
                outer["X-PuttyU-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `puttyu-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `PUTTYU_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `puttyu-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("PUTTYU_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `PUTTYU_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (PUTTYU_INPROCESS_POLLERS=0); "
            "drive `puttyu-mail poll-scheduled` externally."
        )
        return
    import asyncio
