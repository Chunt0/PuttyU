...
def _format_mcp_connection_error(name: str, command: str = "", args: Optional[List[str]] = None, error: Exception = None) -> str:
    ...
    if "@playwright/mcp" in lower_command:
        return (
            ...
            "Then restart puttyU and reconnect the Browser MCP server."
        )
...
class McpManager:
    ...
    async def call_tool(self, qualified_name: str, arguments: Dict) -> Dict:
        ...
        try:
            result = await self._do_call_timed(session, tool_name, arguments)
        except Exception as e:
            # Auto-reconnect for builtin servers whose subprocess may have died
            # (a timeout lands here too, restarting a wedged builtin server).
    ...
    # A wedged MCP server (process alive but unresponsive) must not hang the
    # agent stream forever — the loop awaits the tool with no deadline of its
    # own. Generous ceiling: browser automation can legitimately take minutes.
    TOOL_CALL_TIMEOUT_S = 240

    async def _do_call_timed(self, session, tool_name: str, arguments: Dict) -> Dict:
        """_do_call with a deadline; raises RuntimeError (not bare TimeoutError,
        whose str() is empty and would surface as a blank error message)."""
        try:
            ...
            raise RuntimeError(
                f"MCP tool '{tool_name}' timed out after {self.TOOL_CALL_TIMEOUT_S}s"
            ) from None
