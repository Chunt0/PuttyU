            ...
            if action == "list_tools":
                ...
                return {
                    "response": (
                        ...
                        "memory, skills, images, tasks, notes, calendar."
                    ),
...
async def do_app_api(content: str, owner: Optional[str] = None) -> Dict:
    """Generic loopback to any internal puttyU API endpoint. Lets the
    agent reach the full UI-button surface (cookbook, notes,
    ...
    for t in merged:
        ...
        if t.get("status") == "error" and t.get("output_tail"):
            ...
            if tail:
                ...
                # banner ("Locale: C / Ubuntu_PuttyU ❯") — useless for
...
async def do_tail_serve_output(content: str, owner: Optional[str] = None) -> Dict:
            ...
            for t in (state.get("tasks") or []):
                if isinstance(t, dict) and (t.get("sessionId") == session_id or t.get("id") == session_id):
                    ...
                    if not sport:
                        sport = t.get("sshPort") or ""
                    break
    # Prefer the persisted /tmp/puttyu-tmux/SESSION.log file over the
    ...
    log_path = f"/tmp/puttyu-tmux/{session_id}.log"
