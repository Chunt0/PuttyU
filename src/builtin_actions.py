...
async def action_run_script(owner: str, script: str = "", host: str = "", **kwargs) -> Tuple[str, bool]:
    ...
    target_host = (host or os.environ.get("PUTTYU_SCRIPT_HOST") or os.environ.get("PUTTYU_SCRIPT_HOST") or "localhost").strip()
...
async def action_check_email_urgency(owner: str, **kwargs) -> Tuple[str, bool]:
    ...
    try:
                    ...
                    for uid_b in uids:
                        ...
                        try:
                            ...
                            if not raw:
                                continue
                            msg = _email_mod.message_from_bytes(raw)
                            # Skip puttyU-generated reminders so the scanner
                            ...
                            _ody_origin = (msg.get("X-PuttyU-Origin") or "").strip().lower()
                            _ody_kind = (msg.get("X-PuttyU-Kind") or "").strip().lower()
                            ...
                            if (_ody_origin == "puttyu-ui" or _ody_kind == "reminder"
                                    or _raw_subj.startswith("reminder (puttyu):")
        ...
        if new_urgent:
            ...
            # straight into the puttyU Email tab).
...
BUILTIN_ACTION_INFO = {
    ...
    "run_script": "Run a script locally or on PUTTYU_SCRIPT_HOST",
