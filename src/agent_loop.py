...
Streaming agent loop for puttyu-ui.
...
- "Open/show <panel>" (documents, library, sessions, brain/memories, skills, settings, notes, cookbook) → call `ui_control` with `open_panel <name>`. Panel aliases: library/doc/docs/document→documents, chats/history→sessions, memory/memories→brain, preferences→settings, models/serve/serving→cookbook. CRITICAL: "open memory/memories/brain" / "open skills" / "open notes" / "open documents" / "open cookbook" means OPEN THE PANEL — call `ui_control`, NOT a manage/list tool. The "manage_*" tools list contents in chat; `ui_control open_panel` opens the visual modal the user is asking for.
- User identity facts/preferences ("my name is <name>", "I live in <place>", "I prefer concise replies", "call me <name>") → use `manage_memory` with action=add.
- You are running INSIDE puttyU — there is no OpenWebUI, ChatGPT, or external chat backend to query. All chats/sessions live in THIS app and are accessed via `list_sessions` (or `manage_session` with `action=list`), and deleted via `manage_session` with `action=delete`. Do NOT shell out to find sqlite files, curl localhost:8080, or grep for routers — those don't exist here. If `list_sessions` returns rows, that IS the source of truth.
...
- "Cookbook" = the LLM-serving subsystem (NOT chat sessions, NOT a recipe app). Routing:
  ...
  • "Which servers / GPU boxes are configured" → `list_cookbook_servers`.
  • Launching, stopping, downloading, or adopting model servers is managed from the Cookbook UI — it is not available as an agent tool. If the user asks you to serve/download/stop a model, tell them to use the Cookbook panel.
  ...
  Anti-pattern: POSTing to `/api/cookbook/state` via `app_api` — that overwrites the whole state file (presets and all). Blocked.
...
`calendar` accepts a name ("Main") or short-id prefix.""",
    ...
    "list_served_models": "- ```list_served_models``` — Show what the Cookbook (LLM-serving subsystem) is currently running. NO args. Use this for ANY 'what's running' / 'what's serving' / 'show my cookbook' / 'is anything up' query. DO NOT shell out (`ps aux`, `docker ps`, etc.) — this tool is the source of truth. Failed serve tasks include recent logs plus diagnosis suggestions.",
...
```
GENERIC LOOPBACK to ANY puttyU internal endpoint. Use this whenever the user wants something the UI can do but there's NO named tool for it. Every UI button hits some /api/* endpoint — you can hit the same one. Auth is handled automatically.

**Discovery first.** If you're not sure of the path, call `{"action":"endpoints","filter":"<keyword>"}` (e.g. filter='calendar' or 'notes' or 'theme') to list available endpoints with their methods + summaries. Then call with action='call'.
...
def _resolve_tool_blocks(round_response: str, native_tool_calls: list, round_num: int):
    ...
    Returns (tool_blocks, used_native, failed_native) — failed_native is the
    list of native call names that could not be mapped to any known tool
    (hallucinated/typo'd names). The loop feeds those back to the model so a
    bad name doesn't silently end the turn mid-task.
    """
    used_native = False
    failed_native: list = []
    if native_tool_calls:
        ...
        for tc in native_tool_calls:
            ...
            if block:
                ...
                failed_native.append(tc_name or "(unnamed)")
    ...
    return tool_blocks, used_native, failed_native
...
# ── Completion verifier ── (split into src/agent_verifier.py for Gate 6a;
# imported under the original names so the loop body and tests are unchanged)
from src.agent_verifier import (  # noqa: E402
...
async def stream_agent_loop(
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        tool_blocks, used_native, failed_native = _resolve_tool_blocks(round_response, native_tool_calls, round_num)
        ...
        if not tool_blocks:
            ...
            # The model emitted native tool calls but none mapped to a known
            # tool (hallucinated/typo'd name). Without feedback the turn
            # would silently end here as if the prose were a final answer —
            # mid-task, with the user none the wiser. Tell the model exactly
            # what happened and loop so it can retry with a real tool.
            # Capped so a model that keeps inventing names can't loop forever.
            if (failed_native and not _force_answer
                    and _unknown_tool_nudges < _MAX_UNKNOWN_TOOL_NUDGES):
                _unknown_tool_nudges += 1
                _bad = ", ".join(dict.fromkeys(failed_native))
                logger.info(f"[agent] unknown-tool nudge #{_unknown_tool_nudges} on round {round_num}: {_bad}")
                messages.append({
                    "role": "system",
                    "content": (
                        f"Your tool call(s) failed: no tool named {_bad} exists. "
                        "Use ONLY the tools listed in your instructions, with their "
                        "exact names. Retry now with a valid tool, or answer the "
                        "user directly without tools."
                    ),
                })
        ...
        for i, block in enumerate(tool_blocks):
            ...
            try:
                ...
                # A tool implementation crashed (bug, dead MCP server, bad
                # input). Without this guard the exception kills the SSE
                # generator after tool_start was already emitted: the client
                # never gets tool_output or [DONE] and the UI spins forever.
                # Convert the crash into a normal error result so the model
                # sees the failure and the turn keeps going.
                logger.exception(
                    f"[agent] tool '{block.tool_type}' crashed in round {round_num}"
                )
                desc = f"{block.tool_type} (crashed)"
                result = {
                    "error": f"Tool crashed: {type(_tool_exc).__name__}: {_tool_exc}",
                    "exit_code": 1,
                }
