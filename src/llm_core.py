...
def _ollama_normalize_tool_messages(messages: List[Dict]) -> List[Dict]:
    """Adapt puttyU' canonical OpenAI-style messages to native Ollama /api/chat.

    puttyU carries assistant tool calls in the OpenAI shape, where
...
def _provider_headers(provider: str, headers: Optional[Dict] = None) -> Dict[str, str]:
    ...
    if provider == "openrouter":
        h.setdefault("HTTP-Referer", "https://github.com/Chunt0/PuttyU")
        h.setdefault("X-OpenRouter-Title", "puttyU")
    if provider == "copilot":
...
def _sanitize_llm_messages(messages: List[Dict]) -> List[Dict]:
    """Strip puttyU-only metadata before sending messages to providers.
