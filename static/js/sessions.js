...
let _sortMode = Storage.get('puttyu-session-sort') || 'active'; // default to last active
...
function _deselectCurrentSession(sid) {
  ...
  uiModule.el('current-meta').textContent = 'puttyU Chat';
...
const FOLDER_STATE_KEY = 'puttyu-folder-state';
const FOLDER_ORDER_KEY = 'puttyu-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to puttyU kept
    ...
    if (currentMetaEl) {
      currentMetaEl.textContent = meta ? meta.name : 'puttyU Chat';
    }
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('puttyu-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('puttyu-session-sort', mode);
  else Storage.remove('puttyu-session-sort');
  renderSessionList();
}
