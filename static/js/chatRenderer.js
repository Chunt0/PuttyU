...
const _ACTION_RECENTS_KEY = 'puttyu-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'puttyu-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'puttyU';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'puttyU' : shortModel(resolvedModel);
