...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    const _selKey = 'puttyu-compare-selections-' + (state._compareMode || 'chat');
    let selections = Storage.getJSON(_selKey) || Storage.getJSON('puttyu-compare-selections') || [];
    // Restore synthesis models for search/research
    if (state._compareMode === 'search' || state._compareMode === 'research') {
      const savedSynth = Storage.getJSON('puttyu-compare-synth-' + state._compareMode);
