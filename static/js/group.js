...
const GROUP_STATE_KEY = 'puttyu-group-state';
...
export async function startGroup(models, parentSessionId) {
  ...
  try {
    ...
    try {
      const storedGroupSessions = Storage.getJSON('puttyu-group-sessions', []);
      const gids = Array.isArray(storedGroupSessions) ? storedGroupSessions : [];
      if (!gids.includes(_parentSessionId)) { gids.push(_parentSessionId); localStorage.setItem('puttyu-group-sessions', JSON.stringify(gids)); }
    } catch (e) {}
  } catch (e) {
